...
Elasticsearch 公开了一个成为 *types* （类型）的特性，它允许您在索引中对数据进行逻辑分区。不同 types 的文档可能有不同的字段，但最好能够非常相似。 我们将在 [类型和映射](http://elasticsearch.cn/book/elasticsearch_definitive_guide_2.x/mapping.html) 中更多的讨论关于 types 的一些应用和限制。
